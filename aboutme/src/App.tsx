...
function LoadingScreen({ loading }: { loading: boolean }) {
  return (
    <div
      className={`fixed inset-0 z-50 flex h-dvh w-screen flex-col items-center justify-center bg-[var(--bg)] transition-opacity duration-700 ease-in-out ${
...
function Background({ index }: { index: number }) {
  return (
    <div className="fixed inset-0 -z-10 h-dvh w-screen">
      {slides.map((slide, i) => (
        <div
          key={slide.title}
...
function Carousel({
  ...
  return (
    <div
      className={`w-full overflow-x-hidden overflow-y-visible ${played ? 'relative z-10 mt-2 px-8' : 'my-2 px-6'}`}
    >
      <div
        className={`relative flex items-center justify-start [perspective:1000px] ${played ? 'h-[240px]' : 'h-60'}`}
  ...
  return (
    <>
      ...
      <header className="relative flex items-center justify-between px-6 py-4">
        ...
        <div
          className={`absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center gap-4 rounded-full border border-white/20 bg-black/10 px-5 py-2.5 ${played ? '' : 'backdrop-blur-md'}`}
        >
          <a
      ...
      >
        <div key={index} className="animate-slide-up">
          ...
          {index === 2 && (
            <div className="mt-6 flex max-w-5xl flex-col gap-4 sm:flex-row sm:gap-12">
              <div>
                <div className="text-sm text-white/60">Latest Project</div>
                <div className="mt-1 flex items-center gap-2 text-lg text-white/90">
              ...
              <div>
                <div className="text-sm text-white/60">Primary Stack</div>
                <div className="mt-1 text-lg text-white/90">React + TypeScript + Python</div>
              </div>
            </div>
          )}
          {index === 1 && (
            <div className="mt-6 flex max-w-5xl flex-col gap-4 sm:flex-row sm:gap-12">
              <div>
                <div className="text-sm text-white/60">Current Company</div>
                <div className="mt-1 flex items-center gap-2 text-lg text-white/90">
                  <BriefcaseIcon className="text-white/50" />
                  {experience[0].title}
                </div>
              ...
              <div>
                ...
                <div className="mt-1 text-lg text-white/90">{experience[0].role}</div>
              </div>
              <div>
                <div className="text-sm text-white/60">Experience</div>
                <div className="mt-1 text-lg text-white/90">4+ Years</div>
              </div>
            </div>
          )}
          {!played && (
          ...
          {played && index === 2 && (
            <div className="animate-slide-up mt-6 flex flex-col flex-wrap gap-4 sm:flex-row">
          ...
          {played && index === 3 && (
            <div className="animate-slide-up mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          {played && (index === 4 || index === 5) && (
            <div className="animate-slide-up mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
